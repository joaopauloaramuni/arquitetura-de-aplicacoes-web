...
- 🌌 Buscar a mídia do dia da NASA (imagem ou vídeo)
- 📄 Retornar os dados em formato JSON (DTO)
- 🖼 Exibir imagens diretamente no navegador (HTML)
- 🎬 Exibir vídeos automaticamente (MP4 ou embed como YouTube/Vimeo)

---

## 💡 Observação

A API da NASA pode retornar dois tipos de mídia:

- `image` → exibida com `<img>`
- `video` → pode ser:
    - `.mp4` → exibido com `<video autoplay>`
    - embed (YouTube/Vimeo) → exibido com `<iframe>`

A aplicação trata automaticamente esses cenários.
...
- Título da imagem ou vídeo
- Imagem ou vídeo renderizado
- Descrição
